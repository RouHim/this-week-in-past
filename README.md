...
> **District display (since #209):** Districts (`P:PPLX`) now show hierarchically as `District, City` (e.g. `Bayenthal, Köln` instead of `Bayenthal`, `Christianshavn, København`). The former persistent `geo_location_cache` is auto-dropped on next startup via migration `04` (offline RTree is `<1ms`, no manual `DELETE` needed).
